...
const mockUsers = [
  { id: 1, name: "John Doe", email: "john@example.com", roles: [{ name: "Admin" }] },
  { id: 2, name: "Jane Smith", email: "jane@example.com", roles: [{ name: "User" }] },
];
...
describe("UsersPage Component", () => {
  ...
  beforeEach(() => {
    mockFetchUsers = jest.fn().mockResolvedValue({ data: mockUsers, last_page: 1 });
  ...
  test("updates user role and refreshes data", async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateUserRoles).toHaveBeenCalledWith(2, ["Editor"]);
